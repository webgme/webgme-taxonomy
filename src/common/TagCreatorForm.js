function factory() {
  ...
  class TagCreatorForm {
    ...
    render(data) {
      ...
      this.root.render(React.createElement(Form, data.getProperties(), children));
    }

        async formatData(formatter, formData) {
            formData.taxonomyTags = await formatter.getHumanFormat(formData.taxonomyTags);
            return formData;
        }
    ...
    static inject(_React, _ReactDOM, _JSONSchemaForm) {  // dependency injection
      React = _React;
      ReactDOM = _ReactDOM;
  ...
  class FormRenderData {
    ...
    addButton(name, action, clazz = 'btn-info') {
          this.buttons.push(React.createElement('button', {
              type: 'submit',
              className: `btn ${clazz}`,
              onClick: () => {
    ...
    getProperties() {
      ...
      return {schema: this.schema, onChange, uiSchema: this.uiSchema, formData};
    }
  }
