...
const { DesugarJson } = require("./DesugarJson");
const JSONSchemaExporter = require("./JSONSchemaExporter");
const { SugarLevel } = require("./JSONSchemaExporter");
const Ajv = require("ajv");
const ajv = new Ajv();

class TagFormatter {
  constructor(taxonomy, desugar, validate) {
   ...
   */
  async toGuidFormat(humanTags) {
    // For now, we only use the validator to generate better error messages if
    // desugaring or converting to the GUID format fails. This would probably be
    // good to change but we need to make sure we can allow system terms in the
    // JSON schema.
    try {
      const tags = await this.desugar.desugar(humanTags);
      return this._toGuidFormat(this.taxonomy, tags);
    } catch (err) {
      const valid = this.validate(humanTags);
      if (!valid) {
  ...
  _getGuidTagData(tagNode, data, currentTag) {
    if (Array.isArray(data)) {
      return data.map((datum) => this._getTagData(tagNode, datum, currentTag));
    } else if (typeof data === "object") {
  ...
  static async from(core, taxonomyRoot) {
    ...
    const taxonomyRootTypeName = core.getAttribute(
      core.getBaseType(taxonomyRoot),
      "name",
    );
    assert.equal(
      taxonomyRootTypeName,
      "Taxonomy",
      "Incorrect node passed to TagFormatter. Expected taxonomy root but found: " +
        taxonomyRootTypeName,
    );
    const taxonomy = await load(taxonomyRoot);
    const desugar = new DesugarJson(core, taxonomyRoot);
    const exporter = JSONSchemaExporter.from(
      core,
      taxonomyRoot,
      SugarLevel.Any,
    );
    // TODO: add option for including the base vocab?
    const { schema } = await exporter.getSchemas(taxonomyRoot);
    const validate = ajv.compile(schema);
    return new TagFormatter(taxonomy, desugar, validate);
  }
}
...
class ValidationError extends FormatError {
  constructor(errors) {
    super(
