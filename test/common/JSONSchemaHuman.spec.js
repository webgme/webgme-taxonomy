describe("JSONSchemaExporter", function () {
  ...
  async function getValidateFn(core, root, csv) {
    ...
    console.log(JSON.stringify(schema, null, 2), JSON.stringify(uiSchema, null, 2));
    const validate = ajv.compile(schema);

    return async (humanTag) => {
      return await validate(humanTag);
    };
  }
  ...
  describe("enum", function () {
    ...
    it.only("should support string enums", async function () {
      const taxCsv = `tax,,,
        ,vocab,,,
        ,,enumTerm,,
        ,,,enumProp (enum),
